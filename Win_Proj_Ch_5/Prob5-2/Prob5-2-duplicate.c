            ...
            if (abs(firstPT.x - lastPT.x) > 100 || abs(firstPT.y - lastPT.y) > 100)
            {
                if (abs(firstPT.x - lastPT.x) > abs(firstPT.y - lastPT.y))
                {
                    if (firstPT.x > lastPT.x)
                    {
                        ...
                        if (divideBy == 3)
                            movingTile = search_move_tile((BOOL *)divide3, divideBy, direction);
                        else if (divideBy == 4)
                            movingTile = search_move_tile((BOOL *)divide4, divideBy, direction);
                        else if (divideBy == 5)
                            movingTile = search_move_tile((BOOL *)divide5, divideBy, direction);
                    }
                    else
                    {
                        // 빈 공간 오른쪽에 있는 조각이 이동
...
POINT search_move_tile(BOOL *divide, int divideBy, int direction)
{
    POINT moveTile;
    POINT lastTile;
    for (int i = 0; i < divideBy; ++i)
        for (int j = 0; j < divideBy; ++j)
            if (*(divide + i * divideBy + j))
                lastTile = (POINT){i, j};
