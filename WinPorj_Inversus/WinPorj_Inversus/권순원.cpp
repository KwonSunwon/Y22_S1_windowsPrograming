...
BulletInfo bullet_shoot(Player *, Bullet[], int, int);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    static RECT comboBox;

    static int score;
    static TCHAR scoreText[100];
    static int combo;
    static TCHAR comboText[100];
    ...
    case WM_CREATE:
        ...
        wsprintf(comboText, L"Combo : %d", combo);
    ...
    case WM_TIMER:
        ...
        case OBJECT_MOVE:
            ...
            if (keyBuffer[0x41] || keyBuffer[0x44] || keyBuffer[0x57] || keyBuffer[0x53])
                for (int i = 0; i < MAX_BULLET_DRAW; ++i)
                {
                    if (bullet[i].get_is_live() && bullet[i].get_type() == SPECIAL && !bullet[i].get_is_shoot())
                    {
    ...
    case WM_KEYDOWN:
        if (isGameStart)
        {
            ...
            case 0x4d: // 무적 커맨트
                player.over_power_mode();
            }

            if (isShoot)
            {
                if (bulletCnt < 1 + bulletBonus)
                    if (player.get_bullet_count() != 0)
                    {
                        info = bullet_shoot(&player, bullet, bulletIdx, bulletDirection);
                        ++bulletIdx;
                        if (bulletIdx == MAX_BULLET_DRAW)
                            bulletIdx = 0;
                        if (info._type == BULLET)
                            ++bulletCnt;
                        else if (info._type == SPECIAL)
                            ++bulletBonus;
                    }
                isShoot = FALSE;
            }
        ...
        if (isGameStart)
        {
            ...
            isMainMenu = FALSE;
            player.init(map.get_level());
            SetTimer(hWnd, COMBO_TIMER, COMBO_REDUCE, NULL);
    ...
    case WM_PAINT:
        ...
        else if (isGameStart)
        {
            ...
            DrawText(mdc, comboText, lstrlen(comboText), &uiBox, DT_RIGHT | DT_SINGLELINE | DT_VCENTER);
...
BulletInfo bullet_shoot(Player *_player, Bullet _bullet[], int _idx, int _direction)
{
    BulletInfo info;
